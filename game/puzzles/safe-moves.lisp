...
walls is a list of (row col) pairs."
   ...
   :hint "Compute new position from direction, then check bounds and wall membership."
   :difficulty 2
   :test-cases
   (list
    (make-test-case :input "(safe-move? '(0 0) :right '() 7)" :expected wardlisp-true
                    :description "move right from origin, no walls")
    (make-test-case :input "(safe-move? '(0 0) :up '() 7)" :expected wardlisp-false
                    :description "move up from row 0 is out of bounds")
    (make-test-case :input "(safe-move? '(0 0) :left '() 7)" :expected wardlisp-false
                    :description "move left from col 0 is out of bounds")
    (make-test-case :input "(safe-move? '(3 3) :down '((4 3)) 7)" :expected wardlisp-false
                    :description "move into wall")
    (make-test-case :input "(safe-move? '(6 6) :down '() 7)" :expected wardlisp-false
                    :description "move down from last row is out of bounds")
    (make-test-case :input "(safe-move? '(3 3) :up '((1 1) (5 5)) 7)" :expected wardlisp-true
